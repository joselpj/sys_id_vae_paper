...
vaearx.load_state_dict(torch.load("trained_models/vaearx_wiener_hammer_dataset2021-10-14_00-39-23.dat"))
...
latent_arr = np.load('latent_arr_wiener_hammer.npy')
...
def onclick(event):
   ...
   plt.plot(event.xdata,event.ydata,'k.')

   code = [[float(event.xdata), float(event.ydata)]]
   ...
   def plot_vaearx_reconstruction(y_name, legend_loc):
       ...
       plt.plot(output_arr[0:200], 'r--', label='decoder reconstruction', linewidth=1)
       plt.title('Output')
       plt.xlabel('samples')
       plt.ylabel(y_name)
       plt.grid()
...
ax.scatter(latent_arr[:,0],latent_arr[:,1])
