...
vaearx.load_state_dict(torch.load("trained_models/vaearx_silverbox_dataset2021-11-10_21-16-10.dat"))
...
x_train, y_train, x_test, y_test = silverbox_dataset(4, 5, 1, normalize=False)
test_inputs = torch.Tensor(x_test)
test_outputs = torch.Tensor(y_test)

output = []
samples = test_outputs.shape[0]
columns = 9  # test_inputs.shape[1]

code = [[-0.1,-0.2]]#[[-0.1,0.5]]
z_code = torch.tensor(code)
z_code = z_code.reshape((1, 2))
for i in range(samples):
    x_in = test_inputs[i, 100:109]
...
plot_vaearx_reconstruction(y_name='V2', legend_loc='lower center')

#np.savetxt('decoder_gas_furnace.csv', output_arr, delimiter=',', fmt='%f')
#np.savetxt('dec_cascaded_tanks.csv', test_outputs, delimiter=',', fmt='%f')